...
func (g generator) Generate(gen *protogen.Plugin) error {
	for _, file := range gen.Files {
		...
		generatePathBuilders(g, file, file.Messages)
	}
	return nil
}

func generatePathBuilders(g *protogen.GeneratedFile, f *protogen.File, messages []*protogen.Message) {
	g.P("// Code generated by github.com/kralicky/codegen/pathbuilder. DO NOT EDIT.")
...
var _protopath = protogen.GoImportPath("google.golang.org/protobuf/reflect/protopath")
	...
	for _, fld := range messageFields {
		...
		if fld.Desc.IsList() {
			bx.msgFuncs = append(bx.msgFuncs,
				[]any{"func (p ", unexport(msg.GoIdent.GoName), "PathBuilder) ", fld.GoName, "(idx int) ", unexport(fld.Message.GoIdent.GoName), "PathBuilder {"},
				[]any{" return ", unexport(fld.Message.GoIdent.GoName), "PathBuilder(append(p, ", _protopath.Ident("FieldAccess"), "(((*", msg.GoIdent, ")(nil)).ProtoReflect().Descriptor().Fields().ByNumber(", fld.Desc.Number(), ")), protopath.ListIndex(idx)))"},
				[]any{"}"},
			)
		} else {
			bx.msgFuncs = append(bx.msgFuncs,
	...
	for _, fld := range nonMessageFields {
		...
		if fld.Desc.IsList() {
			bx.fieldFuncs = append(bx.fieldFuncs,
				[]any{"func (p ", unexport(msg.GoIdent.GoName), "PathBuilder) ", fld.GoName, "(idx int) ", _protopath.Ident("Path"), "{"},
				[]any{" return ", _protopath.Ident("Path"), "(append(p, ", _protopath.Ident("FieldAccess"), "(((*", msg.GoIdent, ")(nil)).ProtoReflect().Descriptor().Fields().ByNumber(", fld.Desc.Number(), ")), protopath.ListIndex(idx)))"},
				[]any{"}"},
			)
		} else {
			bx.fieldFuncs = append(bx.fieldFuncs,
